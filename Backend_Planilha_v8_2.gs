...
function brl(v) {
  let p = Number(v || 0).toFixed(2).split('.');
...
function dt(v) {
  return dv(v) ? Utilities.formatDate(v, FUSO, 'dd/MM/yyyy') : '';
}

function dh(v) {
  return dv(v) ? Utilities.formatDate(v, FUSO, 'dd/MM/yyyy HH:mm') : '';
}
...
function limpa() {
  CacheService.getScriptCache().remove(CACHE_CONSULTA);
}
...
function doPost(e) {
  try {
    let p = JSON.parse(e && e.postData && e.postData.contents ? e.postData.contents : '{}');
    ok(p.chave);
    if (p.acao === 'marcar') return out(marcar_(p));
    if (p.acao === 'registrar_pagamento') return out(registrarPagamento_(p));
    if (p.acao === 'anexar_comprovante') return out(anexarComprovante_(p));
    if (p.acao === 'salvar_morador') return out(salvarMorador_(p));
...
function rank_(a) {
  ...
  if (a.indexOf('2ª') >= 0) return 1;
  if (a.indexOf('1ª') >= 0) return 2;
  if (a === 'VENCE HOJE') return 3;
  if (a === 'LEMBRETE') return 4;
  return 5;
}

function chaveH(c, comp, a) {
  return [String(c || '').trim(), ck(comp), String(a || '').trim().toUpperCase()].join('|');
}

function enviadosHoje(ss) {
  let z = new Set(), h = ss.getSheetByName(ABA_HISTORICO);
  if (!h || h.getLastRow() < 4) return z;
  let hoje = Utilities.formatDate(new Date(), FUSO, 'yyyy-MM-dd');
  h.getRange(4, 1, h.getLastRow() - 3, 8).getValues().forEach(function(r) {
    if (dv(r[0]) && Utilities.formatDate(r[0], FUSO, 'yyyy-MM-dd') === hoje && String(r[6] || '').toUpperCase() === 'ENVIADO') {
      z.add(chaveH(r[1], r[3], r[4]));
    }
  });
  return z;
}

function aplicarTemplate_(t, r) {
  return String(t || '')
    .replace(/\[nome\]/gi, r[3])
    .replace(/\[competencia\]/gi, ct(r[1]))
    .replace(/\[valor\]/gi, brl(r[10]))
    .replace(/\[vencimento\]/gi, dt(r[5]));
}

function mensagem_(r) {
  let a = String(r[13] || '').toUpperCase(), ts = templates_().templates,
    t = a === 'LEMBRETE' ? ts.lembrete : a === 'VENCE HOJE' ? ts.vence_hoje : a.indexOf('1ª') >= 0 ? ts.primeira : a.indexOf('2ª') >= 0 ? ts.segunda : '';
  if (t) return aplicarTemplate_(t, r);
  if (String(r[17] || '').trim()) return String(r[17]).trim();
  return aplicarTemplate_(ts.primeira, r);
}

function mapaMoradoresCadastrados_(ss) {
  let s = ss.getSheetByName(ABA_MORADORES);
  let mapa = {
    codigos: new Set(),
    nomes: new Set(),
    lista: []
  };
  if (!s || s.getLastRow() < 4) return mapa;
  let dados = s.getRange(4, 1, s.getLastRow() - 3, 4).getValues();
  dados.forEach(function(r, i) {
    ...
    if (nome && sit.toUpperCase() !== 'INATIVO') {
      if (cod) mapa.codigos.add(cod);
      mapa.nomes.add(nome.toLowerCase());
      mapa.lista.push({ linha: i + 4, codigo: cod, nome: nome, telefone: tel, situacao: sit });
    }
  });
  return mapa;
}
...
function fila_() {
  ...
  x.dados.forEach(function(i) {
    let r = i.row, cod = String(r[2] || '').trim(), m = String(r[3] || '').trim(), t = r[4], st = String(r[12] || '').trim().toUpperCase(), a = String(r[13] || '').trim(), prx = r[15], saldo = Number(r[10] || 0), se = String(r[21] || '').trim().toUpperCase();
    if (!m || !t || st === 'PAGO' || saldo <= 0 || !a || se === 'ENVIADO') return;
    
    // Exibe apenas moradores cadastrados na aba Moradores
    if (temFiltroMoradores) {
      let cadastrado = (cod && mapa.codigos.has(cod)) || mapa.nomes.has(m.toLowerCase());
      if (!cadastrado) return;
    }

    if (se === 'ADIADO' && dv(prx) && prx > ag) return;
    if (hist.has(chaveH(r[2], r[1], a))) return;
    let p = rank_(a);
    if (p <= 2) ven++;
    if (a.toUpperCase() === 'LEMBRETE') lem++;
    total += saldo;
...
function consulta_() {
  ...
  x.dados.forEach(function(i) {
    ...
    // Se houver aba Moradores com cadastros, exibe apenas moradores cadastrados
    if (temFiltroMoradores) {
    ...
    comp = comp || ct(r[1]);
    let devido = Number(r[6] || 0), pago = Number(r[9] || 0), saldo = Number(r[10] || 0), st = String(r[12] || '').trim().toUpperCase(), b = { linha: i.linha, codigo: cod, morador: morador, telefone: r[4] };
    prev += devido; tp += pago;
    if (st === 'PAGO' || saldo <= 0) {
      b.valor_pago = brl(pago);
      b.data_pagamento = dt(r[11]);
      b.forma_pagamento = forma_(r[20]);
      pg.push(b);
    } else {
      b.saldo = brl(saldo);
      b.vencimento = dt(r[5]);
      b.mensagem = mensagem_(r);
      pd.push(b);
      td += saldo;
    }
  });
...
function historico_(codigo) {
  ...
  h.getRange(ini, 1, u - ini + 1, 8).getValues().forEach(function(r) {
    if (String(r[1]) === String(codigo)) {
      z.push({ data: dh(r[0]), competencia: ct(r[3]), tipo: String(r[4] || ''), canal: String(r[5] || ''), resultado: String(r[6] || ''), observacao: String(r[7] || '') });
    }
  });
  z.reverse();
  return { ok: true, historico: z };
}

function data_(t) {
  let m = String(t || '').match(/^(\d{2})\/(\d{2})\/(\d{4})$/);
  if (!m) throw Error('Data inválida. Use dd/mm/aaaa.');
  let d = new Date(+m[3], +m[2] - 1, +m[1]);
  if (d.getFullYear() !== +m[3] || d.getMonth() !== +m[2] - 1 || d.getDate() !== +m[1]) throw Error('Data inválida.');
  return d;
}

function registrarPagamento_(p) {
  ...
  try {
    ...
    limpa();
    return { ok: true, status: quitado ? 'PAGO' : 'PARCIAL', valor_pago: brl(valor), saldo: brl(resta) };
  } finally {
    lock.releaseLock();
  }
}
...
function marcar_(p) {
  ...
  try {
    ...
    if (z === 'ENVIADO' && hist.has(chaveH(r[2], r[1], a))) return { ok: true, duplicado: true };
    if (z === 'ENVIADO') {
      s.getRange(l, 15).setValue(ag);
      s.getRange(l, 17).setValue(Number(r[16] || 0) + 1);
      s.getRange(l, 22).setValue('ENVIADO');
      s.getRange(l, 24).setValue(ag);
      s.getRange(l, 25).clearContent();
    } else if (z === 'ADIAR_1_DIA') {
      let d = new Date(ag);
      d.setDate(d.getDate() + 1);
      s.getRange(l, 16).setValue(d);
      s.getRange(l, 22).setValue('ADIADO');
      s.getRange(l, 25).setValue('Adiado 1 dia pelo aplicativo.');
    } else if (z === 'PAGO') {
      s.getRange(l, 22).setValue('PAGAMENTO INFORMADO');
      s.getRange(l, 25).setValue('Use Registrar pagamento para confirmar valor e data.');
    } else {
      s.getRange(l, 22).setValue(z);
      if (z === 'ERRO') s.getRange(l, 25).setValue('Retentativa necessária.');
    }
    h.appendRow([ag, r[2], r[3], r[1], a, 'WhatsApp', z, String(p.observacao || 'Aplicativo v8.2')]);
    SpreadsheetApp.flush();
    limpa();
    return { ok: true };
  } finally {
...
function templates_() {
  let p = PropertiesService.getScriptProperties(),
    d = {
      lembrete: 'Olá [nome], tudo bem? A Associação de Moradores informa que a contribuição referente a [competencia], no valor de [valor], vencerá em [vencimento]. Agradecemos pela colaboração.',
      vence_hoje: 'Olá [nome], tudo bem? A contribuição da Associação de Moradores referente a [competencia], no valor de [valor], vence hoje. Agradecemos pela colaboração.',
      primeira: 'Olá [nome], tudo bem? A contribuição referente a [competencia], no valor de [valor], permanece em aberto desde [vencimento]. Caso já tenha pago, desconsidere e nos informe. Obrigado.',
      segunda: 'Olá [nome]. Reforçamos que a contribuição referente a [competencia], no valor de [valor], continua pendente. Pedimos a gentileza de regularizar ou entrar em contato. Obrigado.',
      agradecimento: 'Olá [nome], recebemos seu pagamento de [valor] referente a [competencia]. Muito obrigado pela colaboração com a Associação de Moradores!'
    };
  Object.keys(d).forEach(function(k) {
    d[k] = p.getProperty('tpl_' + k) || d[k];
  });
  return { ok: true, templates: d };
}

function salvarTemplates_(p) {
  let t = p.templates || {}, pr = PropertiesService.getScriptProperties();
  ['lembrete', 'vence_hoje', 'primeira', 'segunda', 'agradecimento'].forEach(function(k) {
    if (String(t[k] || '').trim()) pr.setProperty('tpl_' + k, String(t[k]).trim());
  });
  return { ok: true };
}
...
function salvarMorador_(p) {
  ...
  try {
    ...
    if (!linha) {
      let dados = s.getLastRow() >= 4 ? s.getRange(4, 1, s.getLastRow() - 3, 1).getValues() : [], maior = 0;
      dados.forEach(function(r) { maior = Math.max(maior, Number(r[0] || 0)); });
...
function anual_(ano) {
  ...
  s.getRange(4, 1, s.getLastRow() - 3, 25).getValues().forEach(function(r) {
    if (!dv(r[1]) || r[1].getFullYear() !== ano || !r[3]) return;
    let x = meses[r[1].getMonth()];
    x.previsto += Number(r[6] || 0);
    x.pago += Number(r[9] || 0);
    x.pendente += Number(r[10] || 0);
    if (String(r[12] || '').toUpperCase() === 'PAGO' || Number(r[10] || 0) <= 0) x.qtd_pagos++;
    else x.qtd_pendentes++;
  });
  meses.forEach(function(x) {
    x.previsto = brl(x.previsto);
    x.pago = brl(x.pago);
    x.pendente = brl(x.pendente);
  });
  return { ok: true, ano: ano, meses: meses };
}
...
function ativarBackup_() {
  ScriptApp.getProjectTriggers().forEach(function(t) {
    if (t.getHandlerFunction() === 'executarBackupDiario') ScriptApp.deleteTrigger(t);
  });
  ScriptApp.newTrigger('executarBackupDiario').timeBased().everyDays(1).atHour(2).create();
  return { ok: true, mensagem: 'Backup diário ativado para aproximadamente 02:00.' };
}
